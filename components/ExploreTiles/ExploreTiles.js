import React from "react";


export default function ExploreTiles() {
  return (
    <div className="flex flex-row flex-wrap">
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-36 p-7 m-auto ">
    <div className="flex flex-col items-center justify-center bg-gray-400 m-3 rounded-3xl hover:bg-gray-800">
<div className="self-end h-36 w-36 bg-gray-500 m-5 rounded-full flex justify-center items-center">
  <img
    className="h-20 w-20 object-contain p-2"
    src="/knitting.svg"
  />
</div>
<div className="px-5 pb-5">
  <h1 className="font-bold  ">Skill</h1>
  <p className=" ">Name Surname</p>
  <div className="mt-2.5 mb-5 flex items-center">
    <img className="h-30 w-24" src="/arrow.svg" />
    <div className="flex flex-col -mb-12">
    <p>11:30</p>
    <p>Thur 12 Dec</p>
    </div>
  </div>
</div>
</div>
 <div className="flex flex-col items-center justify-center bg-gray-400 m-3 rounded-3xl hover:bg-gray-800">
<div className="self-end h-36 w-36 bg-gray-500 m-5 rounded-full flex justify-center items-center">
  <img
    className="h-20 w-20 object-contain p-2"
    src="/shears.svg"
  />
</div>
<div className="px-5 pb-5">
  <h1 className="font-bold  ">Skill</h1>
  <p className=" ">Name Surname</p>
  <div className="mt-2.5 mb-5 flex items-center">
    <img className="h-30 w-24" src="/arrow.svg" />
    <div className="flex flex-col -mb-12">
    <p>11:30</p>
    <p>Thur 12 Dec</p>
    </div>
  </div>
</div>
</div>
<div className="flex flex-col items-center justify-center bg-gray-400 m-3 rounded-3xl hover:bg-gray-800">
<div className="self-end h-36 w-36 bg-gray-500 m-5 rounded-full flex justify-center items-center">
  <img
    className="h-20 w-20 object-contain p-2"
    src="/piano.svg"
  />
</div>
<div className="px-5 pb-5">
  <h1 className="font-bold  ">Skill</h1>
  <p className=" ">Name Surname</p>
  <div className="mt-2.5 mb-5 flex items-center">
    <img className="h-30 w-24" src="/arrow.svg" />
    <div className="flex flex-col -mb-12">
    <p>11:30</p>
    <p>Thur 12 Dec</p>
    </div>
  </div>
</div>
</div>
<div className="flex flex-col items-center justify-center bg-gray-400 m-3 rounded-3xl hover:bg-gray-800">
<div className="self-end h-36 w-36 bg-gray-500 m-5 rounded-full flex justify-center items-center">
  <img
    className="h-20 w-20 object-contain p-2"
    src="/knitting.svg"
  />
</div>
<div className="px-5 pb-5">
  <h1 className="font-bold  ">Skill</h1>
  <p className=" ">Name Surname</p>
  <div className="mt-2.5 mb-5 flex items-center">
    <img className="h-30 w-24" src="/arrow.svg" />
    <div className="flex flex-col -mb-12">
    <p>11:30</p>
    <p>Thur 12 Dec</p>
    </div>
  </div>
</div>
</div>
<div className="flex flex-col items-center justify-center bg-gray-400 m-3 rounded-3xl hover:bg-gray-800">
<div className="self-end h-36 w-36 bg-gray-500 m-5 rounded-full flex justify-center items-center">
  <img
    className="h-20 w-20 object-contain p-2"
    src="/knitting.svg"
  />
</div>
<div className="px-5 pb-5">
  <h1 className="font-bold  ">Skill</h1>
  <p className=" ">Name Surname</p>
  <div className="mt-2.5 mb-5 flex items-center">
    <img className="h-30 w-24" src="/arrow.svg" />
    <div className="flex flex-col -mb-12">
    <p>11:30</p>
    <p>Thur 12 Dec</p>
    </div>
  </div>
</div>
</div>
<div className="flex flex-col items-center justify-center bg-gray-400 m-3 rounded-3xl hover:bg-gray-800">
<div className="self-end h-36 w-36 bg-gray-500 m-5 rounded-full flex justify-center items-center">
  <img
    className="h-20 w-20 object-contain p-2"
    src="/knitting.svg"
  />
</div>
<div className="px-5 pb-5">
  <h1 className="font-bold  ">Skill</h1>
  <p className=" ">Name Surname</p>
  <div className="mt-2.5 mb-5 flex items-center">
    <img className="h-30 w-24" src="/arrow.svg" />
    <div className="flex flex-col -mb-12">
    <p>11:30</p>
    <p>Thur 12 Dec</p>
    </div>
  </div>
</div>
</div>
<div className="flex flex-col items-center justify-center bg-gray-400 m-3 rounded-3xl hover:bg-gray-800">
<div className="self-end h-36 w-36 bg-gray-500 m-5 rounded-full flex justify-center items-center">
  <img
    className="h-20 w-20 object-contain p-2"
    src="/knitting.svg"
  />
</div>
<div className="px-5 pb-5">
  <h1 className="font-bold  ">Skill</h1>
  <p className=" ">Name Surname</p>
  <div className="mt-2.5 mb-5 flex items-center">
    <img className="h-30 w-24" src="/arrow.svg" />
    <div className="flex flex-col -mb-12">
    <p>11:30</p>
    <p>Thur 12 Dec</p>
    </div>
  </div>
</div>
</div>
<div className="flex flex-col items-center justify-center bg-gray-400 m-3 rounded-3xl hover:bg-gray-800">
<div className="self-end h-36 w-36 bg-gray-500 m-5 rounded-full flex justify-center items-center">
  <img
    className="h-20 w-20 object-contain p-2"
    src="/knitting.svg"
  />
</div>
<div className="px-5 pb-5">
  <h1 className="font-bold  ">Skill</h1>
  <p className=" ">Name Surname</p>
  <div className="mt-2.5 mb-5 flex items-center">
    <img className="h-30 w-24" src="/arrow.svg" />
    <div className="flex flex-col -mb-12">
    <p>11:30</p>
    <p>Thur 12 Dec</p>
    </div>
  </div>
</div>
</div>
<div className="flex flex-col items-center justify-center bg-gray-400 m-3 rounded-3xl hover:bg-gray-800">
<div className="self-end h-36 w-36 bg-gray-500 m-5 rounded-full flex justify-center items-center">
  <img
    className="h-20 w-20 object-contain p-2"
    src="/knitting.svg"
  />
</div>
<div className="px-5 pb-5">
  <h1 className="font-bold  ">Skill</h1>
  <p className=" ">Name Surname</p>
  <div className="mt-2.5 mb-5 flex items-center">
    <img className="h-30 w-24" src="/arrow.svg" />
    <div className="flex flex-col -mb-12">
    <p>11:30</p>
    <p>Thur 12 Dec</p>
    </div>
  </div>
</div>
</div>
<div className="flex flex-col items-center justify-center bg-gray-400 m-3 rounded-3xl hover:bg-gray-800">
<div className="self-end h-36 w-36 bg-gray-500 m-5 rounded-full flex justify-center items-center">
  <img
    className="h-20 w-20 object-contain p-2"
    src="/knitting.svg"
  />
</div>
<div className="px-5 pb-5">
  <h1 className="font-bold  ">Skill</h1>
  <p className=" ">Name Surname</p>
  <div className="mt-2.5 mb-5 flex items-center">
    <img className="h-30 w-24" src="/arrow.svg" />
    <div className="flex flex-col -mb-12">
    <p>11:30</p>
    <p>Thur 12 Dec</p>
    </div>
  </div>
</div>
</div>
        </div>
</div>
  );
}




